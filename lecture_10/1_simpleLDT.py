...
word_1 = df['word_1'].to_list()
word_2 = df['word_2'].to_list()
condition = df['condition'].to_list()
correct_incorrect = df['correct_incorrect'].to_list()
...
for i in range(len(word_1)): # [0, 1, 2, 3, 4]
    trial_num = i + 1
...
data = pd.DataFrame({
    ...
    "participant_name": [participant_name]*len(word_1),
    "participant_age": [participant_age]*len(word_1)
})
